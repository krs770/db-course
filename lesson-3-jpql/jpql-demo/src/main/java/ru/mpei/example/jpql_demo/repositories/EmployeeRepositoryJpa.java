...
import ru.mpei.example.jpql_demo.dto.CitySalary;
import ru.mpei.example.jpql_demo.dto.EmployeeProjects;
...
public class EmployeeRepositoryJpa implements EmployeeRepository {
    ...
    public List<CitySalary> calcAvgSalaryByCities() {
        return em.createQuery(
            "select new ru.mpei.example.jpql_demo.dto.CitySalary(e.address.city, avg(e.salary)) " +
                "from Employee e " +
    ...
    public List<EmployeeProjects> findEmployeesProjectsCount() {
        return em.createQuery(
            "select new ru.mpei.example.jpql_demo.dto.EmployeeProjects(e, count(p)) " +
    ...
    public List<Employee> findEmployeeNameSakes(Employee employee) {
        TypedQuery<Employee> query = em.createQuery(
            "select e " +
                "from Employee e " +
                "where e.firstName in (select e2.firstName from Employee e2 where e2.lastName = " +
                ":lastName and e2.id <> :id) "
            , Employee.class);
    ...
    public Employee findEmployeeNameSake(Employee employee) {
        TypedQuery<Employee> query = em.createQuery(
            "select e " +
                "from Employee e " +
                "where e.firstName = (select e2.firstName from Employee e2 where e2.id = :id) and" +
                " e.id <> :id "
            , Employee.class);
        query.setParameter("id", employee.getId());
    ...
    public int updateEmployeesSalary(BigDecimal oldSalary, BigDecimal newSalary) {
        Query query = em.createQuery(
            "update Employee e set e.salary = :newSalary where e.salary = :oldSalary");
    ...
    public int doubleEmployeesSalary(BigDecimal oldSalary) {
        Query query = em.createQuery(
            "update Employee e set e.salary = e.salary * 2 where e.salary = :oldSalary");
        query.setParameter("oldSalary", oldSalary);
        return query.executeUpdate();
    }
